...
## Old-format email incident (2026-06-19) — root cause + fix

### What happened
3 indeed emails went out today with old format:
- Lindalens Städ och Tilläggstjänster AB — OLD SIGN-OFF (Kasper, Drivverk AB) + OLD OPT-OUT + SPACE-COMMA-SPACE
- R Gruppen Hyr AB — same
- Committo AB — OLD OPT-OUT only (no Drivverk AB, but had "Vill du inte bli kontaktad")

### Root cause
`pipeline_enrich_approve.py` was run BEFORE the 2026-06-18 prompt fixes. Generated bodies
with old format were stored in Supabase. `_doSend()` in emailService.js read `lead.body`
directly and passed it to Resend WITHOUT calling `sanitizeEmail()`. Old pre-generated
bodies went out unchanged even though the code was "fixed".

### Fix (commit 8c56980)
`_doSend()` now calls `sanitizeEmail()` on `lead.body` and `lead.subject` right before
`sendEmail()`. Defense-in-depth: catches any stale em-dash or spacing artifacts in
pre-generated bodies regardless of when they were written to the DB.

### What is NOT fixed retroactively
The 3 emails that already went out. Nothing to do — they were delivered. Recipients saw:
"Kasper, Drivverk AB" (Lindalens, R Gruppen) and old opt-out (all 3).

### Going forward
- New emails generated by current pipeline_enrich_approve.py use correct format
- `_doSend()` sanitizes on the way out as a second safety net
- Run `indeed-intel/fix_stale_indeed_emails.py` if future stale leads appear in queue
...
BCC_EMAIL=cas@casautomations.com
