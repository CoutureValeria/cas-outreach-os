...
## Old-format email incident (2026-06-19) — CLOSED

### What went out with old format (4 emails total)
- Vet-Kamal Djurklinik (veterinary, 07:00 UTC / 09:00 Stockholm) — "Kasper, Drivverk AB" + old opt-out [BCC triggered report]
- Lindalens Städ och Tilläggstjänster AB (indeed, 11:00 UTC) — same
- R Gruppen Hyr AB (indeed, 11:00 UTC) — same
- Committo AB (indeed, 16:38 UTC) — old opt-out only

### Root cause
Pre-generated bodies from before the 2026-06-18 prompt fixes — both Python (indeed) and
...
### Protocol after any prompt change going forward
1. python -X utf8 indeed-intel/fix_stale_indeed_emails.py --dry-run
2. python -X utf8 indeed-intel/fix_stale_vet_emails.py --dry-run
3. Run without --dry-run if stale leads found, BEFORE next send window

### No build caches involved
Python .pyc files are local only — GitHub Actions clones fresh each run.
Backend has no dist/build folder. Railway builds from GitHub source on each push.
The cause was always Supabase stored bodies, not code caching.
